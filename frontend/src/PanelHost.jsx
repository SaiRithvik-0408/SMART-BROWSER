...
const heightFor = (p) => PANEL_HEIGHT[p] || 580;
  ...
  useEffect(() => {
    ...
    const offShow = api.overlay.onShow(({ panel: p, payload: pl }) => {
      ...
      setPanelHistory((prev) => {
        ...
        if (prev.length > 1 && prev[prev.length - 2] === p) {
          return prev.slice(0, -1);
        }
        return [...prev, p];
      });
    });
    const offHide = api.overlay.onHide(() => {
      setPanel(null);
      setPanelHistory([]);
    });
  ...
  const card = (
    <Paper
      ...
      sx={{
        ...
        height: `min(${heightFor(panel)}px, calc(100vh - 120px))`,
    ...
    >
      {panel === 'menu' ? (
        <MenuPanel onClose={close} onOpen={navigateAndClose} onOpenPanel={(name) => api?.overlay?.changePanel(name)} />
      ) : isSheet ? (
        // Notes / VPN render their own header + close button in docked mode.
        <Box sx={{ position: 'relative', flex: 1, minHeight: 0 }}>
          {panel === 'notes' && (
            <NotesPanel open docked onClose={close} onBack={handleBack} initialNoteId={payload?.initialNoteId ?? null} />
          )}
          {panel === 'vpn' && <VpnPanel open docked onClose={close} onBack={handleBack} />}
        </Box>
      ) : (
        <>
          <Box sx={{
            ...
            <IconButton size="small" onClick={handleBack} sx={{ color: '#9aa3c7', mr: 0.5 }}>
